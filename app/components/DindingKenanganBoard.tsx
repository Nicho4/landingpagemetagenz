...
import type { GuestNote, NoteColor, WashiPos } from "./DindingKenangan";
  ...
  return (
    ...
    >
      ...
      {/* Pending badge — kept in case a future admin-preview view ever
          ...
          ever receives approved notes from the server, so this never shows there. */}
      {note.pending && (
...
export function DindingKenanganBoard({ initialNotes }: { initialNotes: GuestNote[] }) {
  const prefersReducedMotion = !!useReducedMotion();

  // Data papan sekarang datang langsung dari server (props) — sudah "siap"
  // sejak render pertama, jadi tidak perlu lagi fetch("/api/notes") di
  // useEffect terpisah, dan tidak perlu flag notesLoaded buat nunggu itu
  // selesai. Papan tidak pernah kelihatan kosong sesaat lalu keisi lagi.
  // Daftar ini juga tidak pernah berubah lagi setelah mount (submit baru
  // masuk sebagai "pending", tidak langsung tampil di papan ini), jadi
  // tidak butuh useState — cukup pakai nilai props apa adanya.
    ...
    <section ref={sectionRef} className="relative py-16 md:py-24 bg-[#F6F0E6]">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          {/* Empty state — data selalu sudah siap dari render pertama
              (props dari server), jadi tidak perlu lagi flag notesLoaded
              buat nunggu fetch client selesai. */}
